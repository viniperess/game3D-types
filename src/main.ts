...
const jet = await objLoader.loadAsync(objFile) as any;
const jetJoystick: { x: number | null;
                      y: number | null; 
                    } = {
                      x: null,
                      y: null,
...
function moveEnemy(enemy: Enemy): void {
...
function fire() {
  ...
  shot.model.position.set(jet.position.x, jet.position.y, jet.position.z)
...
function shootDown(enemy: Enemy): boolean {
  if (jet.shots.length == 0) return false;
  return jet.shots.find((shot: Shot) => shot.hit.intersectsSphere(enemy.hit))
}

function showEnemyHit(enemy: Enemy): void {
...
function updateJoystick(event: MouseEvent): void {
  if (!event.buttons) {
